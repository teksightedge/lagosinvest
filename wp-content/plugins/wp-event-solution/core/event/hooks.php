...
class Hooks {
	...
	public function event_column_headers( $columns ) {
		...
		if ( class_exists('Wpeventin_Pro') && $attendee_registration ) {
			$new_array['view_attendee']    = esc_html__('View Attendee',  'eventin' );
		}
	...
	public function event_column_data( $column, $post_id ) {
		...
		switch ( $column ) {
			...
			case 'view_attendee' : 
				?>
				<a href="<?php echo esc_url(admin_url('edit.php?post_type=etn-attendee&event_id='.intval( $post_id )))?>">Attendees</a>
				<?php
			break; 
		}
	...
	public function form_submit_visibility( $visible, $post_id ) {
		...
		$event_expire_start_time    = !empty( get_post_meta( $post_id, "etn_start_time", true ) ) && !is_null( get_post_meta( $post_id, "etn_start_time", true ) ) ? get_post_meta( $post_id, "etn_start_time", true ) : "";

		if ( $selected_expiry_point == "start" ) {
				//event start date-time
				$event_expire_date      = $event_expire_start_date;
				$event_expire_time      = $event_expire_start_time;
				...
				$event_expire_time      = !empty( get_post_meta( $post_id, "etn_end_time", true ) ) && !is_null( get_post_meta( $post_id, "etn_end_time", true ) ) ? get_post_meta( $post_id, "etn_end_time", true ) : $event_expire_start_time;
		}

		$event_expire_time      = $event_expire_time == "" ? '24:00' : '';
